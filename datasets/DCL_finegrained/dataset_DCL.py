...
class dataset(data.Dataset):
    def __init__(self, Config, anno, swap_size=[7,7], common_aug=None, swap=None, totensor=None, train=False, train_val=False, test=False):
        ...
        if isinstance(anno, pandas.core.frame.DataFrame):
            ...
            self.labels = (anno['label']-1).tolist()
        elif isinstance(anno, dict):
            self.paths = anno['img_name']
            self.labels = anno['label']-1
        ...
        if self.dataset == 'CUB':
            ...
            for dir_name in os.listdir(self.root_path):
                for img_name in os.listdir(os.path.join(self.root_path, dir_name)):
                    if img_name in self.paths:
                        self.img_names[img_name] = dir_name
    ...
    def __getitem__(self, item):
        if self.dataset == 'CUB':
            img_path = os.path.join(self.root_path, self.img_names[self.paths[item]], self.paths[item])
        else:
...
def collate_fn4test_path(batch):
    imgs = []
    label = []
    path = []
    for sample in batch:
        imgs.append(sample[0])
        label.append(sample[1])
        path.append(sample[-1])
    return torch.stack(imgs, 0), label, path
