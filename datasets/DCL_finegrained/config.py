...
try:    
    ...
    from DCL_finegrained import transforms
...
class LoadConfig(object):
    def __init__(self, data, dataset, swap_num, backbone, version):
        ...
        if dataset == 'CUB':
            ...
            self.rawdata_root = f'{DATA_ROOT_DIR}/CUB/CUB_200_2011/CUB_200_2011/images'
            self.anno_root = 'datasets/DCL_finegrained/datasets/CUB'
            self.numcls = 200
        elif dataset == 'STCAR':
            self.dataset = dataset
            self.rawdata_root = f'{DATA_ROOT_DIR}/STCAR'
            self.anno_root = 'datasets/DCL_finegrained/datasets/STCAR'
            self.numcls = 196
        elif dataset == 'AIR':
            self.dataset = dataset
            self.rawdata_root = f'{DATA_ROOT_DIR}/AIR/fgvc-aircraft-2013b/data/images'
            self.anno_root = 'datasets/DCL_finegrained/datasets/AIR'
            self.numcls = 100
        else:
